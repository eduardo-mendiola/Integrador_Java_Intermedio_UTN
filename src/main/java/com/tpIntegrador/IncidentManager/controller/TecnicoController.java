...
public class TecnicoController {
    ...
    */
    ...
    public ResponseEntity<Tecnico> findOneById(@Parameter(description = "Clave primaria tipo Long") @PathVariable Long id) {
        //@PathVariable vincula id con {id}

        Optional<Tecnico> tecOpt = tecnicoRepository.findById(id); //
        // @PathVariable
    ...
    @Operation(summary = "Crear un técnico", description = "Con este método podemos dar de alta a un técnico en la base" +
            " de datos.")
    public ResponseEntity<Tecnico> create(@RequestBody Tecnico tecnico, @RequestHeader HttpHeaders headers){
        ...
        if (tecnico.getId() != null){ // Quiere decir que existe el id, y por lo tanto no es una creación.
    ...
    public ResponseEntity<Tecnico> update(@RequestBody Tecnico tecnico) {
        if(tecnico.getId() == null){ // si no tiene id quiere decir que si es una creación.
            log.warn("Trying to update a non existent technical");
            return ResponseEntity.badRequest().build();
        }
        if(!tecnicoRepository.existsById(tecnico.getId())) {
            log.warn("Trying to update a non existent technical");
            return ResponseEntity.notFound().build();
        }
    ...
    public ResponseEntity<Tecnico> deleteAll() {
